...
<div class="w-full h-screen bg-[url('https://i.ibb.co/hZpdbkY/248479853-2133515060137518-4885646735485668126-n.jpg')] bg-cover bg-center text-white">
    ...
    <div class="grid grid-cols-4 gap-4">
        ...
        <div class="border-2 shaddow-inner p-4 rounded-lg drop-shadow-xl w-full bg-[#989a9e]/95 text-black">
                <div class="text-[16px]">
                ...
                <a href="{{ route('votepage.id', $item->id) }}" class="flex justify-end ">
                    <div class="mt-2 w-1/2 p-1 rounded hover:bg-[#fabc02] bg-white">
                        <p class="text-center hover:text-white text-black font-bold">ลงคะเเนนเสียง</p>
                    </div>
                    </a>
                    @else
                    <div class="mt-2 w-1/2 p-1 rounded hover:bg-[#fabc02] bg-white cursor-no-drop">
                        <p class="text-center hover:text-white text-black font-bold">กรุณาเข้าสู่ระบบก่อนเข้าใช้งาน</p>
                    </div>
                @endif
        </div>
        @endforeach
