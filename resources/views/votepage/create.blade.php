...
@section('content')
<div class="border-2 w-[32rem] p-2 text-between mx-auto shadow-xl mt-10 bg-[#adadad]/85">
    ...
    <div class="flex justify-center mt-8 ">
        ...
        <div class="gap-6">
            ...
            @if($item->id == 1)
            <button name="vote_id" value="{{ $item->id == 1}}" class="btn bg-[#34eb4c] ml-6 p-2 rounded hover:bg-white hover:text-green-500 border-2 border-green-500 font-bold " type="submit">เห็นด้วย</button>
            @else
            <button name="vote_id" value="{{ $item->id == 2}}" class="btn bg-[#eb4034] ml-6 p-2 rounded hover:bg-white hover:text-red-500 border-2 border-red-500 font-bold " type="submit">ไม่เห็นด้วย</button>
            @endif
            @endforeach
            @error('vote_id')
            <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
